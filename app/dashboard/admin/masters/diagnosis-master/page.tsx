...
const categoryColors: Record<string, string> = {
  Endocrine: 'bg-green-100 text-green-700',
  Cardiovascular: 'bg-red-100 text-red-700',
  Respiratory: 'bg-blue-100 text-blue-700',
  Digestive: 'bg-yellow-100 text-yellow-700',
  Musculoskeletal: 'bg-purple-100 text-purple-700',
  Neurological: 'bg-indigo-100 text-indigo-700',
  Psychiatric: 'bg-pink-100 text-pink-700',
  Genitourinary: 'bg-gray-100 text-gray-700',
};
...
export default function DiagnosisMaster() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/diagnosis-master?page=${page}&pageSize=${PAGE_SIZE}&search=${encodeURIComponent(search)}&category=${encodeURIComponent(category)}`)
      .then(res => res.json())
      .then(data => {
        setRows(data.rows);
        setTotal(data.total);
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold leading-tight">Diagnosis Master</h1>
              <div className="text-gray-500 text-sm">Manage diagnostic codes and descriptions</div>
            </div>
          </div>
        </div>
        {/* Controls */}
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-700">
                ...
                <th className="px-4 py-2 text-left font-semibold">DIAGNOSIS NAME</th>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-2 py-3 text-xs text-gray-500">
          <div className="mb-2 sm:mb-0">
            {rows.length > 0 && (
              <span>
                Showing {((page-1)*PAGE_SIZE)+1} to {((page-1)*PAGE_SIZE)+rows.length} of {total} diagnoses
              </span>
            )}
          </div>
